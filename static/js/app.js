...
window.onload = async () => {
    ...
    //seulemnt initialiser graphs quand on load page la premiere fois
    ...
    document.querySelector('#choix-pays-activer').onclick = () => {
        document.querySelector('#choix-pays-liste').classList.toggle('active')
    }
...
showConfirmedTotal = (total) => {
    document.querySelector('#total-confirmes').textContent = numberWithCommas(total)
}

showRecoveredTotal = (total) => {
    document.querySelector('#total-guerisons').textContent = numberWithCommas(total)
}

showDeathsTotal = (total) => {
    document.querySelector('#total-deces').textContent = numberWithCommas(total)
}
...
loadSummary = async (country) => {
   ...
   let casesByCountries = summaryData.Countries.sort((a, b) => b.TotalConfirmed - a.TotalConfirmed)
    ...
    let table_countries_body = document.querySelector('#table-pays tbody')
...
initAllTimesChart = async () => {
    ...
    all_time_chart = new ApexCharts(document.querySelector('#toute-la-periode'), options)
...
initDaysChart = async () => {
    ...
    days_chart = new ApexCharts(document.querySelector('#graph-jours'), options)
...
initRecoveryRate = async () => {
    let options = {
        ...
        series: [],
        labels: ['Taux de guerisons'],
        colors: [COLORS.recovered]
    }

    recover_rate_chart = new ApexCharts(document.querySelector('#graph-taux-gurisons'), options)
...
renderCountrySelectList = (list) => {
    let country_select_list = document.querySelector('#choix-pays-liste')
    country_select_list.querySelectorAll('div').forEach(e => e.remove())
    list.forEach(e => {
        let item = document.createElement('div')
        item.classList.add('pays-item')
        item.textContent = e.Country

        item.onclick = async () => {
            document.querySelector('#choix-pays span').textContent = e.Country
...
loadCountrySelectList = async () => {
    ...
    item.onclick = async () => {
        document.querySelector('#choix-pays span').textContent = 'Global'
        country_select_list.classList.toggle('active')
        await loadData('Global')
    }
    country_select_list.appendChild(item)
...
// pays triés par
initCountryFilter = () => {
    let input = document.querySelector('#choix-pays-liste input')
    input.onkeyup = () => {
